...
for veg, group in vegetable_groups:
    ...
    avg_price = group['Average'].mean()
    Y_train = (group['Average'] > avg_price).astype(int)
    
    grid_search = GridSearchCV(RandomForestClassifier(), param_grid, cv=KFold(n_splits=2, shuffle=True), scoring='accuracy')
...
binary_test = (y_test > average_price).astype(int)
accuracy = accuracy_score(binary_test, predictions_df.mean(axis=1).round())
